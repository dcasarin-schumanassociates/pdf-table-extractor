import cv2

def detect_table_cells(preprocessed_img):
    img_draw = cv2.cvtColor(preprocessed_img, cv2.COLOR_GRAY2BGR)
    inv_img = 255 - preprocessed_img

    vertical_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (1, preprocessed_img.shape[0] // 100))
    vertical_lines = cv2.dilate(cv2.erode(inv_img, vertical_kernel, iterations=2), vertical_kernel, iterations=2)

    horizontal_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (preprocessed_img.shape[1] // 100, 1))
    horizontal_lines = cv2.dilate(cv2.erode(inv_img, horizontal_kernel, iterations=2), horizontal_kernel, iterations=2)

    table_mask = cv2.add(horizontal_lines, vertical_lines)
    contours, _ = cv2.findContours(table_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    boxes = []
    for cnt in contours:
        x, y, w, h = cv2.boundingRect(cnt)
        if w > 30 and h > 20:
            boxes.append((x, y, w, h))
            cv2.rectangle(img_draw, (x, y), (x+w, y+h), (0, 255, 0), 1)

    return img_draw, sorted(boxes, key=lambda b: (b[1], b[0]))  # sort top to bottom, then left to right

